...
if [[ $1 = "deploy" ]]; then
  ...
  # enable route forwarding (if you cannot access docker instance from your host)
  sysctl net.ipv4.conf.all.forwarding=1
  sudo iptables -P FORWARD ACCEPT
  # you will need to execute on you physical computer
  # route add 172.26.0.0/28 192.168.1.62 #(IP of your virtual machine)


  docker rm -f `docker ps -a | grep sparkbase | awk '{ print $1 }'` # delete old containers
  docker network rm sparknet
  docker network create --driver bridge --subnet=172.26.0.0/28 sparknet # create custom network
